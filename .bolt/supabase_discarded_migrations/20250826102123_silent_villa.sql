...
-- This script attempts to drop and recreate all necessary enums, tables, and functions
-- to ensure the database schema is correctly set up.

-- Set search_path to public for simplicity
SET search_path TO public;

-- Enable uuid-ossp extension if not already enabled
-- This is crucial for uuid_generate_v4() function
...
DECLARE
    ...
    uuid_extension_enabled boolean;
BEGIN
    ...
    SELECT EXISTS (SELECT 1 FROM pg_extension WHERE extname = 'uuid-ossp') INTO uuid_extension_enabled;

    RAISE NOTICE 'Schema rebuild completed!';
    RAISE NOTICE 'Enum types created: %', enum_count;
    RAISE NOTICE 'Tables created: %', table_count;
    RAISE NOTICE 'Functions created: %', function_count;
    RAISE NOTICE 'uuid-ossp extension enabled: %', uuid_extension_enabled;

    IF enum_count = 4 AND table_count = 11 AND function_count = 3 AND uuid_extension_enabled THEN
