...
export function PriceListPrintView({ 
  ...
  // Funzione per generare template HTML email con colori FARMAP
  const generateEmailHTML = (emailBody: string, customerName?: string, priceListName?: string): string => {
...
<body style="margin: 0; padding: 0; font-family: 'Inter', -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, 'Helvetica Neue', Arial, sans-serif; background-color: #f3f4f6;">
  ...
  const handleSendEmailComplete = async (emailData: { email: string; subject: string; body: string }) => {
    ...
    try {
      ...
      // Genera HTML formattato per l'email
      const htmlBody = generateEmailHTML(
        emailData.body,
        priceList.customer?.company_name || 'Cliente',
        priceList.name
      );
